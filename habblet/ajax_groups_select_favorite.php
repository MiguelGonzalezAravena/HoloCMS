...
if($groupid > 0) {
	...
	if($exists > 0) {
		...
		if($already_member > 0) {
			mysqli_query($connection, "UPDATE groups_memberships SET is_current = '0' WHERE userid = '{$my_id}'") or die(mysqli_error($connection));
			mysqli_query($connection, "UPDATE groups_memberships SET is_current = '1' WHERE userid = '{$my_id}' AND groupid = '{$groupid}' LIMIT 1") or die(mysqli_error($connection));
		} else {
			exit;
		}
	} else {
