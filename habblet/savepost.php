...
if(!empty($topicId)) {
  ...
  if($exists > 0) {
    ...
    $valid_thread = ($thread['type'] != 2 && $thread['type'] != 4) ? true : false;
  } else {
...
if($valid_thread == true) {
  mysqli_query($connection, "INSERT INTO cms_forum_posts(threadid, message, author, date) VALUES('{$topicId}', '{$message}', '{$name}', '{$date_full}')");
  mysqli_query($connection, "UPDATE users SET postcount = postcount + 1 WHERE id = '{$my_id}' LIMIT 1") or die(mysqli_error($connection));
  mysqli_query($connection, "UPDATE cms_forum_threads SET lastpost_author = '{$name}', lastpost_date = '{$date_full}', unix = '{$now}', posts = posts + 1 WHERE id = '{$topicId}' LIMIT 1") or die(mysqli_error($connection));
}
    ...
    <div class="postlist-header clearfix">
      ...
      <?php echo ($user_rank > 5 ? '<a href="#" id="edit-topic-settings" class="edit-topic-settings-link">Moderation Tools &raquo;</a><input type="hidden" id="settings_dialog_header" value="Moderation Tools" />' : ''); ?>
        <div class="page-num-list">
          <input type="hidden" id="current-page" value="<?php echo $page; ?>" /> View page:
          <?php
            for ($i = 1; $i <= $pages; $i++) {
              if($page == $i) {
                echo $i . '\n';
        ...
        <tr class="post-list-index-<?php echo $oddeven; ?>">
          <a id="post-<?php echo $row['id']; ?>">
            <td class="post-list-row-container">
              ...
              <div class="clearfix">
                <div class="post-list-creator-avatar"><img src="http://www.habbo.co.uk/habbo-imaging/avatarimage?figure=<?php echo $userdata['figure']; ?>&size=b&direction=2&head_direction=2&gesture=sml" alt="<?php echo $userdata['name']; ?>" /></div>
                <div class="post-list-group-badge">
                  <?php echo (GetUserGroup($userid) != false ? '<a href="' . $path . 'group_profile.php?id=' . GetUserGroup($userid) . '"><img src="' . $path . 'habbo-imaging/badge.php?badge=' . GetUserGroupBadge($userid) . '" /></a>' : ''); ?>
                </div>
                <div class="post-list-avatar-badge">
                  <?php echo (GetUserBadge($userid) != false ? '<img src="http://images.habbohotel.co.uk/c_images/album1584/' . GetUserBadge($userid) . '.gif" />' : ''); ?>
                </div>
              </div>
            ...
            <td class="post-list-message" valign="top" colspan="2">
              ...
              <div class="post-list-content-element">
                <?php echo bbcode_format(trim(nl2br(HoloText($row['message'])))); ?>
