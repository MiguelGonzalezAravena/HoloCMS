...
$pane = isset($_POST['forumType']) ? FilterText($_POST['forumType']) : '';
$topic = isset($_POST['newTopicPermission']) ? FilterText($_POST['newTopicPermission']) : '';
...
<?php 
  ...
  mysqli_query($connection, "UPDATE groups_details SET name = '{$name}', description = '{$description}', type = '{$type}', pane = '{$pane}', topics = '{$topic}', roomid = '{$roomId}' WHERE id = '{$groupid}' AND ownerid = '{$my_id}' LIMIT 1") or die(mysqli_error($connection));
?>
