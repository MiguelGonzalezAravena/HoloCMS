...
|+===================================================+
|| # Copyright © 2016 Miguel González Aravena. All  reserved.
|| # https://github.com/MiguelGonzalezAravena/HoloCMSrights
|+===================================================+
...
switch($mode) {
  case 1:
    $str = 'friends';
    $get_em = mysqli_query($connection, "SELECT friendid FROM messenger_friendships WHERE userid = '{$my_id}' OR friendid = '{$my_id}' ORDER BY friendid LIMIT 500") or die(mysqli_error($connection));
    break;
  case 2:
    $str = 'groups';
...
<?php
    ...
    while($row = mysqli_fetch_assoc($get_em)) {
      ...
      $even2 = (IsEven($num)) ? 'odd' : 'even';
?>
  <li class="' . $even2 . '">
<?php if($row['is_current'] == 1) { ?>
    <div class="favourite-group" title="Favourite"></div>
<?php
      }
...
<?php } ?>
    <a href="<?php echo $path; ?>group_profile.php?id=<?php echo $group_id; ?>"><?php echo HoloText($groupdata['name']); ?></a>
  </li>
<?php } ?>
</ul>
<?php } elseif($mode == 3) { ?>
...
<?php
    while ($row = mysqli_fetch_assoc($get_em)) {
      ...
      $even = (IsEven($oddeven)) ? 'odd' : 'even';
?>
  <li class="<?php echo $even; ?>"><a href="<?php echo $path; ?>client.php?forwardId=2&amp;roomId=<?php echo $row['id']; ?>" onclick="roomForward(this, '<?php echo $row['id']; ?>', 'private'); return false;" target="client" id="room-navigation-link_<?php echo $row['id']; ?>"><?php echo $row['name']; ?></a></li>
<?php } ?>
</ul>
<?php
  } else {
...
if($mode == 3) {
?>
<p class="create-room"><a href="<?php echo $path; ?>client.php?shortcut=roomomatic" onclick="HabboClient.openShortcut(this, 'roomomatic'); return false;" target="client">Create a new room</a></p>
<?php } elseif($mode == 2) { ?>
